...
valcore logfire push support-golden-set`}</Code><p>In the interface, use the corresponding <strong>Import from Logfire</strong> and <strong>Push to Logfire</strong> actions. Pushing does not require the tracing token; importing does not require the write key.</p>
    ...
    <section id="experiments"><h2>Experiment Run touchpoints</h2><p>The <strong>tracing token</strong> is the only Logfire credential required to sync an Experiment Run. In <strong>Runs → New run</strong>, select the Dataset and Agent Evaluator, choose <strong>Validation</strong>, and enable <strong>Run as a Logfire experiment</strong>.</p><Code>{`valcore experiment <agent-evaluator> <dataset>`}</Code><p>Valcore executes the cases through <code>pydantic_evals.Dataset.evaluate</code>. The Experiment Run appears in Logfire&apos;s Evals view with an experiment span, case spans, task execution, evaluator assertions, inputs, expected labels, actual judgments, durations, and traces. Valcore also keeps its local run record and statistical results.</p><p>Logfire-backed Experiment Runs cannot be cancelled or retried row by row. Start a new Experiment Run when configuration or individual cases need to change.</p>
      <Note title="Validation needs ground truth">Every Dataset row must have a confirmed label before Valcore enables a validation Experiment Run. An unlabeled Dataset can still be used for an evaluation run, but it cannot produce agreement statistics.</Note>
    </section>
