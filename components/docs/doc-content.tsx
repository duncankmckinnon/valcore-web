...
import { VALCORE_SKILL_SOURCE_URL, VALCORE_SKILLS_DOWNLOAD_URL } from "@/lib/links";
...
function GettingStarted() {
  return <>
    ...
    <section id="local-agent"><h2>Local agent</h2><h3>Using the CLI agent in Valcore</h3><p>Install Valcore&apos;s bundled skill in the repository where your coding agent works:</p><Code>{`cd your-agent-project\nvalcore skills install`}</Code><p>The default installs to <code>.agents/skills/</code>. Use <code>--claude</code>, <code>--copilot</code>, <code>--all</code>, or <code>--global</code> when you need a different destination.</p><h3>Download from GitHub</h3><p>The website can also package the current <code>src/valcore/skills/use-valcore</code> directory from GitHub&apos;s <code>main</code> branch. The resulting ZIP contains only the skill, not the rest of the repository.</p><p><a className="docs-download" href={VALCORE_SKILLS_DOWNLOAD_URL} download>Download Valcore skills (.zip) <span aria-hidden="true">↓</span></a></p><p><a href={VALCORE_SKILL_SOURCE_URL}>Inspect the skill source on GitHub <span aria-hidden="true">→</span></a></p><Note title="Choose one installation path">The CLI installer is best when Valcore is already installed because it handles Codex, Claude, Copilot, global targets, updates, and symlinks. The ZIP is a portable setup path for reviewing or installing the GitHub-hosted skill manually.</Note><p>The app and CLI use the same local SQLite workspace, so no Valcore server needs to be running for agent-driven CLI work. Resources can be addressed by name or a unique ID prefix.</p><Code>{`valcore list evaluators\nvalcore list datasets\nvalcore list runs`}</Code><p>Continue with the <a href="/docs/cli">CLI &amp; automation reference</a> for commands, portable packages, and CI.</p></section>
  </>;
}
