...
import { AnnotationsPanelVisual, DatasetPanelVisual, EvaluatorPanelVisual, RunsPanelVisual } from "./panel-visuals";
...
function Datasets() {
  return <>
    ...
    <section id="dataset-panel"><h2>Dataset panel tour</h2><p>The Datasets panel is both a catalog and a readiness check. The list shows how much evaluation material exists and whether it is labeled enough for validation; opening a dataset switches to the row-level authoring surface.</p><DatasetPanelVisual />
      <ol className="docs-steps"><li><strong>Use the summary before opening anything.</strong> Dataset count and total rows show coverage; the labeled percentage tells you how close the workspace is to supporting validation.</li><li><strong>Choose the creation path that matches your source.</strong> Blank is for hand-authored cases, Upload accepts CSV/JSON, Generate creates synthetic cases, and Logfire imports hosted datasets or queried traces.</li><li><strong>Read labeled status as a run constraint.</strong> “Complete” means every row has ground truth and can be used for validation. Partially labeled datasets remain usable for ordinary evaluation runs.</li><li><strong>Work row data in the detail view, labels in Annotate.</strong> The detail grid edits case fields inline, adds rows, and deletes cases that do not belong. Select <strong>Annotate</strong> to apply or correct labels—see <a href="/docs/annotations">Annotations</a>.</li></ol>
      <p>The detail panel&apos;s stats header shows total/labeled/unlabeled counts and label distribution for the dataset&apos;s primary Label Set. Generated and Logfire-sourced datasets retain collapsible provenance panels so you can see the instructions or SQL that created them.</p>
    </section>
    <section id="adding"><h2>Adding datasets</h2><p>Start from a blank table, upload existing examples, query real traces, sync a hosted dataset, or generate synthetic cases.</p>
      <h3>Blank entries</h3><p>Select <strong>New dataset → Blank</strong>, enter a name, add the columns the Agent Evaluator will receive, and optionally define a label schema. Providing one here creates the dataset&apos;s first <a href="/docs/annotations#label-sets">Label Set</a> immediately, ready to annotate. After creation, use <strong>Add row</strong> in the grid and edit each cell inline. This is the most direct route for a small golden set or a handful of deliberate edge cases.</p>
      <h3>CSV or JSON upload</h3><p>Select <strong>New dataset → Upload</strong>. Valcore accepts CSV, JSONL, or a Valcore eval-package JSON file, infers the data columns, and previews the result before creation. Keep one case per CSV row or JSONL object. If labels are present, identify the label column and provide the matching categorical or numeric schema—Valcore creates a Label Set from it and seeds each row&apos;s annotation.</p><Note title="Inspect inferred columns">Upload is intentionally a preview-first flow. Check that identifiers, metadata, and labels were not accidentally inferred as evaluator inputs before creating the dataset.</Note>
      ...
      <h3>Generating synthetic datasets</h3><h4>Creating a dataset from specs</h4><p>Describe the behavior to test, choose the exact columns, and add per-column notes. You can request suggested labels and, for categorical schemas, opt into a target label mix. Review the editable draft before saving.</p><Note title="Suggestions are not ground truth">Generated labels are stored as suggestions on the dataset&apos;s Label Set. Accept or correct them in <a href="/docs/annotations">Annotations</a> before using the dataset for validation.</Note>
    ...
    <section id="labeling"><h2>Labeling datasets</h2><p>Evaluation runs can use unlabeled data. Validation runs require every row to have a ground-truth label from a Label Set whose kind and labels match the evaluator&apos;s score contract.</p>
      <p>Defining a label schema when creating a dataset—Blank, Upload, Generate, or Logfire—creates that dataset&apos;s first Label Set automatically and, for generated or imported rows, seeds each row&apos;s suggested label. From there, confirming suggestions, correcting them, and adding further Label Sets to the same dataset all happen in <a href="/docs/annotations">Annotations</a>, not in the Dataset grid.</p>
    </section>
...
function Annotations() {
  return <>
    <PageIntro eyebrow="Ground truth" title="Annotations">Define what an annotator records for a Dataset, then work its rows through a keyboard-driven queue that shows suggestions, source, and progress.</PageIntro>
    <section id="overview"><h2>Label Sets and Annotations</h2><p>A <strong>Label Set</strong> is an annotation contract: a name, a kind (categorical or numeric), and either the fixed labels an annotator can choose from—each with its own criteria description—or a numeric range. An <strong>Annotation</strong> is one row&apos;s recorded labels or value under one Label Set, along with an optional description and, when generation or import supplied one, a suggested value it can be checked against.</p><AnnotationsPanelVisual />
      <p>A Dataset can carry more than one Label Set—for example a primary quality judgment plus a secondary sentiment check—each tracked and annotated independently. The stats header on a Dataset&apos;s detail page reflects its primary Label Set; open <strong>Annotate</strong> from there, or start from the Annotations panel, to reach any of them.</p>
    </section>
    <section id="label-sets"><h2>Creating and managing Label Sets</h2><p>Select <strong>Annotations</strong>, choose a Dataset, then <strong>New label set</strong>. Name it, choose Categorical or Numeric, and either add labels—each with the criteria text shown to annotators as reference during review—or set a minimum and maximum.</p><p>Providing a label schema while creating a Dataset (see <a href="/docs/datasets#adding">Adding datasets</a>) creates its first Label Set for you. Add further Label Sets here at any time; each starts with every row unannotated.</p></section>
    <section id="queue"><h2>The Annotation Queue</h2><p>Opening a Label Set from the Annotations panel loads a table over its Dataset&apos;s rows, one hundred at a time, with columns for the row data, current labels, any suggestion, its source, and a description field. Expand a row to see its full column values, edit a cell inline, or delete the row entirely.</p>
      <h3>Keyboard shortcuts</h3><p>The queue is built for keyboard-first review: <kbd>j</kbd>/<kbd>k</kbd> move the focused row, <kbd>1</kbd>–<kbd>9</kbd> toggle the Nth categorical label on the focused row (multi-select, not replace), <kbd>a</kbd> accepts its suggestion, <kbd>u</kbd> clears its annotation, <kbd>Enter</kbd> opens it in the <a href="/docs/annotations#row-view">single-row view</a>, and <kbd>?</kbd> shows this list in the app. Every change saves immediately with an optimistic update that rolls back if the request fails.</p>
      <h3>Suggestions and acceptance</h3><p>Rows from a generated or imported Dataset can carry a suggested label or value alongside the confirmed one, shown in the Suggested column. Press <kbd>a</kbd> or select <strong>Accept</strong> to copy it into the row&apos;s Annotation; the Source column then reads <strong>accepted</strong> rather than <strong>manual</strong> or <strong>generated</strong>.</p><Note title="Suggestions are not ground truth">A suggested label reflects what generation produced or import inferred, not human review. Accept or correct every row before relying on the Label Set for a validation run.</Note>
    </section>
    <section id="row-view"><h2>The single-row view</h2><p>Press <kbd>Enter</kbd> on a focused row, or open a row directly, for a full-page view of its data, labels, and description with <strong>Prev</strong>/<strong>Next</strong> buttons over the same loaded page—useful for careful, one-row-at-a-time review rather than the denser queue table.</p></section>
    <section id="validation"><h2>Feeding Validation runs</h2><p>A validation Experiment Run needs a Label Set on the chosen Dataset whose kind and, for categorical sets, whose exact labels match the Agent Evaluator&apos;s score contract. Valcore reports how many of the Dataset&apos;s rows that matching Label Set has annotated, and Validation is unavailable until every row is covered. See <a href="/docs/experiments#validation">Validation Experiment Runs</a> for how those labels are compared against Agent Evaluator scores.</p></section>
  </>;
}
